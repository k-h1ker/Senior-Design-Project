...
data_training = pd.read_csv('whites_d1_1mm_boot.csv')  #we train on the boot strapped data
data_test = pd.read_csv('whites_d1_1mm.csv')           #we test on the actual
...
def prediction(x_calib, y_calib, x_valid, y_valid, plot_components=False):
    ...
    #f = open("whites_d1_1mm_model.txt", "w+")
    #m = pickle.dumps(pls)
    #f.write(m)
